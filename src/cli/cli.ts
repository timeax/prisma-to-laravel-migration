...
type StubType = 'migration' | 'model' | 'enum' | 'ts';
...
function generatorBlock(
   ...
   const provider = `prisma-laravel-${name}`; // prisma-laravel-migrations / prisma-laravel-models
...
cli
   ...
   .action(async (opts) => {
      ...
      const stubDirRel =
         './' + path.relative(process.cwd(), userStubs).replace(/\\/g, '/'); // "./stubs"
      ...
      // --- TS types generator (no stubs; relies on its own defaults) ---
      if (!hasTypesGen) {
         schema += `
generator types {
...
`;
      ...
      /* 5. Create prisma-laravel.config.js if absent ---------------- */
...
cli
   ...
   .action(
      async (opts: {
         ...
         const wantAll =
            !opts.migrations && !opts.models && !opts.enums;
         ...
         if (wantAll || opts.migrations) {
            const files = ls(out.migrations, (f) =>
               f.endsWith('.php'),
            );
            console.log('\n📦 Migrations:');
            ...
            if (opts.sorted) {
               const bakDir = path.join(
                  out.backups,
                  path.relative(process.cwd(), out.migrations),
               );
               const bakFiles = existsSync(bakDir)
                  ? readdirSync(bakDir).filter((f) =>
                     f.endsWith('.php'),
                  )
                  : [];
               if (!bakFiles.length) {
                  ...
                  const ordered = [...bakFiles].sort((a, b) =>
   ...
   .action(
      async (opts: {
         ...
         const want = new Set(
            (opts.types?.length
               ? opts.types
               : ['migration', 'model', 'enum']) as ('migration' | 'model' | 'enum')[],
         );
         const names = new Set(
