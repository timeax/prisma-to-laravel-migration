...
<details>
<summary>TS <code>stubs/ts/index.stub</code> (module-level)</summary>
...
#### Stub variables (module-level)

* `imports: string` → merged import block for all **non-stubbed** models.
* `body: string` → full `declare module "…" { … }` wrapper generated by the TS printer.
* `moduleName: string` → the configured module name (default `"database/prisma"`).
* `models: TsModelDefinition[]` → full list of model definitions (including stubbed ones).

#### `content` (module-level)

`content` is a **callable string-like function** that preserves old behavior:

* If you interpolate it directly (`${content}`), it behaves like the old string:
  it outputs the default generated model chunks (interfaces/types only).
* If you call it, you can **patch/override specific model chunks** without rewriting
  the whole generator output.
...
So in the module-level stub you receive:
...
Example: add a property to one model without rebuilding everything:
...
${content((m) =>
  m.name === "Account"
    ? { body: { addProps: ["// added in stub", "locale?: string"] } }
    : undefined
)}
```
...
### TypeScript model stubs `<Model>.stub`

<details>
<summary>TS <code>stubs/ts/&lt;Model&gt;.stub</code> (per-model)</summary>
...
* `model` → the current model definition.
* `imports` → import block for that model.
* `content` → same callable string-like function, scoped to **this single model**
  (so `content()` returns the default chunk for just that model).
* `body` → the default chunk wrapped in `declare module "…" { … }`.
* `moduleName` → module name string.

Example (per-model): append a helper type after the generated interface:
